...
def read_data(data_file, table_file):  # data文件，数据表文件
    data, tables = [], {} # 数据，表格
    with open(data_file) as f: # 读取问题信息
        for l in f:
            data.append(json.loads(l))
    with open(table_file) as f: # 读取表格信息
        for l in f:
            l = json.loads(l)
            d = {}
            ...
            #############
            d['type'] = l['types'] # column types
            d['headers+type']=[]
            for i in range(len(d['headers'])):
                d['headers+type'].append(d['headers'][i]+','+d['type'])  # 合并
            d[headers+type2id] = {j: i for i, j in enumerate(d['headers+type'])}
            #############
            d['content'] = {}
            d['all_values'] = set()
            rows = np.array(l['rows'])
            for i, h in enumerate(d['headers']):
                d['content'][h] = set(rows[:, i]) # set 负责去重
                d['all_values'].update(d['content'][h])
            d['all_values'] = set([i for i in d['all_values'] if hasattr(i, '__len__')])
            tables[l['id']] = d  # l['id']是表格编号 将所有表格信息汇总到tables={}
    return data, tables
...
class data_generator:
    ...
    def __iter__(self): # 每次迭代
        while True:
            ...
            for i in idxs:
                d = self.data[i] # d
                #t = self.tables[d['table_id']]['headers']  # 数据表header 能不能换成header+type
                t1 = self.tables[d['table_id']]['headers']  # 数据表header 能不能换成header+type
                t2 = self.tables[d['table_id']]['types']  # 数据表header 能不能换成header+type
                t_all=[]
                for i in range(len(t1)):
                   t_all.append(t1[i]+','+t2[i])
                
                x1, x2 = tokenizer.encode(d['question']) # x1,x2 代表自然问题编码
                xm = [0] + [1] * len(d['question']) + [0]  # x 的mask 表示
                h = [] # 列名
                for j in t_all: # j 来自table # t换成t_all
                    _x1, _x2 = tokenizer.encode(j) # 关于数据表头表示编码
                    h.append(len(x1)) # [7，7，7，7]
...
x1_in = Input(shape=(None,), dtype='int32') # 实例化张量 bert的问题文本输入 shape=(?,?)
x2_in = Input(shape=(None,)) #实例化张量 bert的问题文本输入 shape=(?,?)
xm_in = Input(shape=(None,)) #实例化张量 输入序列的mask  shape=(?,?)
h_in = Input(shape=(None,), dtype='int32') #实例化张量 列名所在位置  shape=(?,?)
hm_in = Input(shape=(None,))  #实例化张量 列名mask  shape=(?,?)
sel_in = Input(shape=(None,), dtype='int32')   #实例化张量 被select的列  shape=(?,?)
conn_in = Input(shape=(1,), dtype='int32') #实例化张量 连接类型  shape=(?,1)
#csel_in = Input(shape=(None,), dtype='int32')  #实例化张量 条件中的列 shape=(?,?)
csel_in = Input(shape=(None,),dtype='int32')  #实例化张量 条件中的列 shape=(?,?)
cop_in = Input(shape=(None,), dtype='int32') #实例化张量 条件中的运算符（同时也是值的标记）shape=(?,?)
...
x4conn = Lambda(lambda x: x[:, 0])(x)  # 对于bert的输入的处理 shape=(?,768)  # 直接获取cls位置
...
pcsel = Dense(1)(pcsel)
pcsel = Lambda(lambda x: x[0][..., 0] - (1 - x[1]) * 1e10)([pcsel, hm])  # mask 机制 softmax机制
...
def nl2sql(question, table):
    ...
    for i in range(len(t1):
       t_all.append(t1[i]+','+t2[i])
    for i in t_all: 能添加type信息 table['header']换成t_all
